...
import { HospitalService } from 'src/app/admin/Hospital/services/hospital.service';
import { MatSnackBar } from '@angular/material';

@Component({
  selector: 'app-filter-blood-bank',
  templateUrl: './filter-blood-bank.component.html',
  styleUrls: ['./filter-blood-bank.component.css']
})
export class FilterBloodBankComponent implements OnInit {

  constructor(private fb: FormBuilder, private adminBloodBankService: BloodBankService, private snackBar: MatSnackBar) { }
  ...
  @Output() addBloodBank = new EventEmitter<any>();
  // @Output() addBloodType = new EventEmitter<any>();
  AddState: string;
  Addcity: string;
  AddBloodType: string;
  modifyBloodBankArea: any;
  modifyArea: any;
  sortArray = [];
  ngOnInit() {
  ...
  bloodBankName() {
    this.addBloodBank.emit(this.form2.controls.bloodBankName.value);
  }
  ...
  // bloodTypewiseSearch() {
  //   this.addBloodType.emit(this.form1.controls.bloodGroup.value);
  // }
  clickCity() {
